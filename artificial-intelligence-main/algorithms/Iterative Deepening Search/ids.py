...
def iterative_deepening_search(root, goal_value):
    depth = 0
    while True:
        result = depth_limited_search(root, goal_value, depth)
...
goal_value = 6
result = iterative_deepening_search(root, goal_value)

if result is not None:
    print(f"Goal value {goal_value} found!")
else:
    print(f"Goal value {goal_value} not found.")
